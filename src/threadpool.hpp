...
std::array<int, sizeof...(Args)> log(Args &&..._args) {
...
template <typename F, typename... Args> struct TaskExecutor : TaskBase {
...
public:
  ...
  ThreadPool(const uint worker_count = std::thread::hardware_concurrency(),
      ...
      : worker_count{worker_count}, buffer_size{buffer_size} {
